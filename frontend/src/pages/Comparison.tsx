...
export default function ComparisonPage() {
  ...
  return <div className="funnel-page content-page"><Helmet><title>{title}</title><meta name="description" content={`How does ${item.competitor} compare with Elemental Bond? A neutral comparison of method, free access, subscription status, and disclosed price.`} /><link rel="canonical" href={`${SITE_URL}/compare/${key}`} /></Helmet>
    ...
    <div className="comparison-table" role="table"><div role="row"><strong>Criterion</strong><strong>{item.competitor}</strong><strong>Elemental Bond</strong></div><div role="row"><span>Method</span><span>{item.method}</span><span>BaZi-inspired Five Elements relationship patterns</span></div><div role="row"><span>Free layer</span><span>{item.free}</span><span>Core-element result and relationship preview without required email</span></div><div role="row"><span>Subscription</span><span>{item.subscription}</span><span>Early-access list only; no recurring charge in the validation release</span></div><div role="row"><span>Disclosed price</span><span>Check the provider's current listing</span><span>$14.90 USD for Decode This Connection</span></div></div>
