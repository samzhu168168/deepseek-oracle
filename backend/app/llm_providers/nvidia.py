...
class NvidiaProvider(BaseLLMProvider):
    """NVIDIA NIM provider — free tier via build.nvidia.com.

    Base URL: https://integrate.api.nvidia.com/v1
    API key prefix: nvapi-

    Recommended free models (2026):
      - nvidia/nemotron-3-super-120b-a12b   (best overall agentic, 442 tok/s)
      - nvidia/gpt-oss-120b                  (best function-calling + reasoning)
      - qwen/qwen3.5-397b-a17b              (best bilingual CN/EN)
      - zai-org/glm-5.1                      (best Chinese agentic)
      - deepseek-ai/deepseek-v4-pro          (best pure reasoning)
    """

    def __init__(self, api_key: str, base_url: str, model: str = "nvidia/gpt-oss-120b"):
        super().__init__(model=model)
        if not OPENAI_AVAILABLE:
            raise ImportError("OpenAI module not installed. NVIDIA provider requires 'openai' package.")
        self.client = OpenAI(api_key=api_key, base_url=base_url)

    def generate(self, user_message: str, timeout_s: int = 120) -> LLMResult:
