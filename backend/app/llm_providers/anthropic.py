...
class AnthropicProvider(BaseLLMProvider):
    """Claude provider via OpenAI-compatible relay (OpenRouter, NanoBanana, etc.).

    Set ANTHROPIC_BASE_URL to your relay endpoint:
      - OpenRouter:    https://openrouter.ai/api/v1
      - NanoBanana:    your configured NANOBANANA_BASE_URL
      - Direct:        https://api.anthropic.com/v1 (requires Anthropic-native SDK)

    The relay must present an OpenAI-compatible /chat/completions endpoint.
    """

    def __init__(self, api_key: str, base_url: str, model: str = "claude-sonnet-4-6"):
        super().__init__(model=model)
        if not OPENAI_AVAILABLE:
            raise ImportError("OpenAI module not installed. Anthropic relay provider requires 'openai' package.")
        self.client = OpenAI(api_key=api_key, base_url=base_url)

    def generate(self, user_message: str, timeout_s: int = 90) -> LLMResult:
        ...
        response = self.client.chat.completions.create(
            ...
            stream=False,
            timeout=timeout_s,
            # B.AI / OpenAI-compatible relays don't need extra_headers.
            # If you switch to OpenRouter you can re-add:
            # extra_headers={"HTTP-Referer": "https://elemental.bond", "X-Title": "Elemental Bond Oracle"},
        )
